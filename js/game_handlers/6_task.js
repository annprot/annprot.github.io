...
function start_game() {
	...
	document.getElementById("task").innerHTML = "Выпишите лишнее слово из предложения или замените неверно употреблённое:"
	$("#exercs").fadeOut(1000);
	$("#faq").fadeOut(1000);
	$("#game").fadeIn(1000);
}
...
function set_data(datajson) {
	...
	for(var i = 0; i < data.words.length; i++) {
		...
		p_words.set(pair_words[0].trim(), pair_words[1]);

		window.rwords.push(pair_words[0].trim());
	}
...
function game_handle() {
	...
	if(document.getElementById("us_answer").value.toLowerCase().trim() == p_words.get(rwords[0])) {
		flag = true;
		rwords.shift();
	...
	if(!flag) {
		...
		document.getElementById("us_answer").placeholder = p_words.get(rwords[0]);
